...
window.onload = () => {
  ...
  let planetPadding = (((container.clientHeight / 2) - (ORBITING_PLANET_SIZE * NUM_PLANETS)) / 7);
  ...
  let initSpeed = () => {
    ...
    animating = true;
    container.style.animation = "fade-out-in 1s ease";
    setTimeout(() => {
    ...
    setTimeout(() => {container.style.animation = null; animating = false;}, 1000);
  };
...
class Planet extends AstronomicalObject {
  ...
  setOrbitPath() {
    ...
    this.orbitPath.style.left = this.x - (width / 2) + (ORBITING_PLANET_SIZE / 2) + "px";
    this.orbitPath.style.top = this.y + (ORBITING_PLANET_SIZE / 2) + "px";
